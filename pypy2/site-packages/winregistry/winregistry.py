...
class WinRegistry(object):
    ...
    def read_value(self, key, value, key_wow64_32key=False):
        ...
        '''
        try:
            handle = self._get_handle(key, _winreg.KEY_READ, key_wow64_32key)
            reg_value = _winreg.QueryValueEx(handle, value)
            handle.Close()
        except:
            raise
    ...
    def write_value(self, key, value, data=None, reg_type='REG_SZ', key_wow64_32key=False):
        ...
        '''
        try:
            handle = self._get_handle(key, _winreg.KEY_SET_VALUE, key_wow64_32key)
            _winreg.SetValueEx(handle, value, 0, getattr(_winreg, reg_type), data)
            handle.Close()
        except:
            raise
    ...
    def read_key(self, key, key_wow64_32key=False):
        ...
        try:
            handle = self._get_handle(key, _winreg.KEY_READ, key_wow64_32key)
            keys_num, values_num, modify = _winreg.QueryInfoKey(handle)
            resp['modify'] = datetime(1601, 1, 1) + timedelta(microseconds=modify/10)
        except:
            raise

        for key_i in range(0, keys_num):
            resp['keys'].append(_winreg.EnumKey(handle, key_i))

        for key_i in range(0, values_num):
            value = {}
            value['value'], value['data'], value['type'] = _winreg.EnumValue(handle, key_i)
    ...
    def create_key(self, key, key_wow64_32key=False):
        ...
        while (i < len(subkeys)) and (not handle):
            try:
                ...
                handle = self._get_handle(current, _winreg.KEY_WRITE, key_wow64_32key)
            except FileNotFoundError:
                i += 1

        tail = '\\'.join(subkeys[len(subkeys) - i:])
        _winreg.CreateKeyEx(handle, tail, 0, self._get_access(_winreg.KEY_WRITE))
        handle.Close()
    ...
    def delete_key(self, key, key_wow64_32key=False):
        ...
        try:
            ...
            handle = self._get_handle(parental, _winreg.KEY_WRITE, key_wow64_32key)
    ...
    def _get_handle(self, key, access, key_wow64_32key=False):
        ...
        try:
            if root != self.root or not self.root_handle:
                self.root_handle = _winreg.ConnectRegistry(self.host, root)
            key_handle = _winreg.OpenKey(self.root_handle, path, 0, access)
        except:
            raise
    ...
    def _get_access(self, access, key_wow64_32key=False):
        x64_key = _winreg.KEY_WOW64_32KEY if key_wow64_32key else _winreg.KEY_WOW64_64KEY
    ...
    def _parse_root(path):
        ...
        try:
            ...
            reg_root = getattr(_winreg, _root)
        except:
